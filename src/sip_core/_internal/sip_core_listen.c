...
cme_error_t __SipCore_listen(sip_core_reqh_t reqh, struct GenericPtr arg,
  ...
  queue__SipCoreListeners_push(sip_core.get->__listeners,
                               (struct __SipCoreListener){
                                   .reqh = reqh,
                                   .arg = arg,
                               });

  err = SipTransportPtr_listen(sip_core.get->__sip_transp,
...
static inline cme_error_t __SipCore_sip_transp_recvh(
  ...
  if (queue__SipCoreListeners_is_empty(sip_core.get->__listeners)) {
    return 0;
  }
  ...
  if (SipMessagePtr_is_request(sip_msg)) {
    ...
    result =
        __SipServerTransactions_find(branch, sip_core.get->__stranses, &strans);
    if (!result) {
